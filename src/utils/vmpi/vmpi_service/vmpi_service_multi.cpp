...
// list of all the processes we've started
CUtlVector<PROCESS_INFORMATION> g_RunningProcesses;
...
// the default distributor is effectively limited to 4 threads per worker
// database tracking, vmpi_job_watch, etc are also configured to only show 4 threads
// work around this by launching more workers
...
bool g_Waiting_bShowAppWindow = true;
...
void KillRunningProcess( const char *pReason, bool bGoToIdle )
{
	...
	for (int i = 0; i < g_RunningProcesses.Count(); i++)
	{
		if (g_RunningProcesses[i].hProcess)
		{
			TerminateProcess(g_RunningProcesses[i].hProcess, 1);
			CloseHandle(g_RunningProcesses[i].hThread);
			CloseHandle(g_RunningProcesses[i].hProcess);
		}
	}
	g_RunningProcesses.Purge();
...
PROCESS_INFORMATION RunProcessAtCommandLine( 
	CUtlVector<char*> &newArgv, 
	bool bShowAppWindow,
...
{
	if (g_RunningProcesses.Count() == 0)
		return false;

	bool bAllProcessesExited = true;
	
	// Check if all processes have exited
	for (int i = 0; i < g_RunningProcesses.Count(); i++)
	{
		if (WaitForSingleObject(g_RunningProcesses[i].hProcess, 0) == WAIT_TIMEOUT)
		{
	...
	else
	{
		...
		Msg("All worker processes finished!\n");
...
void GetRunningProcessStats(int &processorPercentage, int &memoryUsageMegabytes)
{
	...
	if (g_RunningProcesses.Count() > 0)
	{
		...
		if ((curTime - lastReturnTime) >= 1000)
		{
			...
			// For simplicity, we'll just track the first process
			// A more complete solution would track all processes and sum their usage
	...
	bool bPatching )
{
	*hProcess = NULL;
	...
	// removed for now because we want to keep the binaries in the cache directory
...
bool CheckDownloaderFinished()
{
	...
	else
	{
		...
		{
			// Random port offset for multiple workers
			//char *portArg = g_Waiting_Argv[2];
			//if (portArg)
			//{
			//	char ipAddress[128];
			//	char portStr[32];
			...
			//	const char *colonPos = V_strstr(portArg, ":");
			//	if (colonPos)
			//	{
			//		// Copy the IP portion
			//		int ipLen = colonPos - portArg;
			//		V_strncpy(ipAddress, portArg, ipLen + 1);
			//		ipAddress[ipLen] = '\0';
			//		
			//		// Generate a random port offset for this worker
			//		int randomOffset = RandomInt(0, g_NumWorkers);
			//		int basePort = atoi(colonPos + 1);
			//		int newPort = basePort + randomOffset;
			//		
			//		// Format the new port string
			//		V_snprintf(portStr, sizeof(portStr), "%s:%d", ipAddress, newPort);
			//		
			//		// Replace the original argument
			//		delete[] g_Waiting_Argv[2];
			//		g_Waiting_Argv[2] = CopyString(portStr);
			//		
			//		Msg("Worker %d using port: %s\n", i+1, portStr);
			//	}
			//}

			PROCESS_INFORMATION pi = RunProcessAtCommandLine( g_Waiting_Argv, g_Waiting_bShowAppWindow, g_Waiting_bPatching, g_Waiting_Priority );

			if (pi.hProcess)
			{
				g_RunningProcesses.AddToTail( pi );
				Msg( "Started worker %d on process ID %d\n", i+1, pi.dwProcessId );
			}
		}
		
		// Store the first process ID for performance tracking
		if (g_RunningProcesses.Count() > 0)
		{
			g_dwRunningProcessId = g_RunningProcesses[0].dwProcessId;
			g_pPerfTracker->Init(g_dwRunningProcessId);
			g_CreateProcessTime = GetTickCount();
		}
		...
		{                                                      
			...
			// Resume the first thread (installer)
			if (g_RunningProcesses.Count() > 0)
				ResumeThread(g_RunningProcesses[0].hThread);
...
void HandlePacket_KILL_PROCESS( const CIPAddr *ipFrom )
{
	...
	{
		KillRunningProcess( "Got a KILL_PROCESS packet. Stopping all worker executables.\n", true );
                     ...
                     int       nCmdShow)
{
	...
	const char *pNumWorkersArg = FindArg( __argc, __argv, "-mpi_NumWorkers", NULL );
	if ( pNumWorkersArg )
	{
		int numWorkers = atoi( pNumWorkersArg );
